...
protected:
	void SetUp() override { button = std::make_unique<Button>(10, 10, 120, 40, "Click Me"); }
...
TEST_F(ButtonTest, Constructor_SetsLabel) { EXPECT_EQ(button->GetLabel(), "Click Me"); }
...
TEST_F(ButtonTest, IsEnabled_DefaultsToTrue) { EXPECT_TRUE(button->IsEnabled()); }
...
TEST_F(ButtonTest, OnClick_WithinBounds_TriggersCallback) {
	...
	MouseEvent event{50, 25, false, false, true, false, 0.0f};
...
TEST_F(ButtonTest, OnClick_OutsideBounds_DoesNotTriggerCallback) {
	...
	MouseEvent event{200, 200, false, false, true, false, 0.0f};
	button->OnClick(event);
...
TEST_F(ButtonTest, OnHover_UpdatesHoverState) {
	...
	MouseEvent hover_event{50, 25, false, false, false, false, 0.0f};
	button->OnHover(hover_event);

	EXPECT_TRUE(button->IsHovered());

	// Mouse exits button bounds
	MouseEvent exit_event{200, 200, false, false, false, false, 0.0f};
