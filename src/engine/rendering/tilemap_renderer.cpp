...
TilemapRenderer::TilemapRenderer() { current_batch_.Reserve(max_batch_size_); }
...
void TilemapRenderer::Render(
	const components::Tilemap& tilemap,
...
void TilemapRenderer::BuildLayerBatch(
	const std::shared_ptr<components::TilemapLayer>& layer,
...
void TilemapRenderer::AddTileToBatch(
	...
	batch.indices.insert(
		batch.indices.end(),
		{
			...
			vertex_offset + 2,
			vertex_offset + 3,
			vertex_offset + 0 // Second triangle
		}
	);
...
void TilemapRenderer::RenderBatch(const TileBatch& batch, const glm::mat4& mvp_matrix, ShaderManager& shader_manager) {
	...
	glBindVertexArray(vao_);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_);
	glBufferSubData(GL_ARRAY_BUFFER, 0, batch.vertices.size() * sizeof(TileVertex), batch.vertices.data());
