...
std::unique_ptr<IPhysicsBackend> CreatePhysicsBackend(PhysicsEngineType engine) {
	switch (engine) {
	case PhysicsEngineType::JoltPhysics: return CreateJoltBackend();
	case PhysicsEngineType::Bullet3: return CreateBullet3Backend();
	case PhysicsEngineType::PhysX: return CreatePhysXBackend();
	default: throw std::runtime_error("Unknown physics engine type");
	}
}
