...
struct MouseEvent {
	...
	explicit MouseEvent(const engine::input::MouseState& state) :
			x(state.x), y(state.y), left_down(state.left_down), right_down(state.right_down),
			middle_down(state.middle_down), left_pressed(state.left_pressed), right_pressed(state.right_pressed),
			middle_pressed(state.middle_pressed), left_released(state.left_released),
			right_released(state.right_released), middle_released(state.middle_released),
