...
export namespace engine::ui::elements {
    ...
    public:
        ...
        Checkbox(float x, float y, const std::string& label = "", 
                 float font_size = 16.0f, bool initial_checked = false)
            : UIElement(x, y, font_size * 1.5f, font_size * 1.5f)  // Square box
            , is_checked_(initial_checked)
            , box_size_(font_size * 1.5f)
            , label_text_(label)
            , font_size_(font_size)
            , box_color_(batch_renderer::Colors::LIGHT_GRAY)
            , checkmark_color_(batch_renderer::Colors::GOLD)
            , label_color_(batch_renderer::Colors::WHITE)
            , focus_color_(batch_renderer::Colors::GOLD)
            , label_spacing_(8.0f)
         ...
         */
        void SetBoxColor(const batch_renderer::Color& color) {
            box_color_ = color;
        }
         ...
         */
        void SetCheckmarkColor(const batch_renderer::Color& color) {
            checkmark_color_ = color;
        }
         ...
         */
        void SetFocusColor(const batch_renderer::Color& color) {
            focus_color_ = color;
        }
        ...
        void Render() const override {
            ...
            Color box_outline = is_hovered_ 
                ? Color::Brightness(box_color_, 0.2f)
                : box_color_;
            
            // Top edge
            BatchRenderer::SubmitLine(box_rect.x, box_rect.y, 
                                     box_rect.x + box_rect.width, box_rect.y,
                                     border_width, box_outline);
            // Right edge
            BatchRenderer::SubmitLine(box_rect.x + box_rect.width, box_rect.y,
                                     box_rect.x + box_rect.width, box_rect.y + box_rect.height,
                                     border_width, box_outline);
            // Bottom edge
            BatchRenderer::SubmitLine(box_rect.x + box_rect.width, box_rect.y + box_rect.height,
                                     box_rect.x, box_rect.y + box_rect.height,
                                     border_width, box_outline);
            // Left edge
            BatchRenderer::SubmitLine(box_rect.x, box_rect.y + box_rect.height,
            ...
            if (is_focused_) {
                const float focus_padding = -4.0f;  // Outside box
                ...
                const float focus_width = 1.0f;
                // Top edge
                BatchRenderer::SubmitLine(focus_rect.x, focus_rect.y,
                                         focus_rect.x + focus_rect.width, focus_rect.y,
                                         focus_width, focus_color_);
                // Right edge
                BatchRenderer::SubmitLine(focus_rect.x + focus_rect.width, focus_rect.y,
                                         focus_rect.x + focus_rect.width, focus_rect.y + focus_rect.height,
                                         focus_width, focus_color_);
                // Bottom edge
                BatchRenderer::SubmitLine(focus_rect.x + focus_rect.width, focus_rect.y + focus_rect.height,
                                         focus_rect.x, focus_rect.y + focus_rect.height,
                                         focus_width, focus_color_);
                // Left edge
                BatchRenderer::SubmitLine(focus_rect.x, focus_rect.y + focus_rect.height,
    ...
    private:
        ...
        // Appearance
        batch_renderer::Color box_color_;
        batch_renderer::Color checkmark_color_;
        batch_renderer::Color label_color_;
        batch_renderer::Color focus_color_;
