...
public:
	ClassRegistration(std::string name, IScriptingBackend* backend) : class_name_(std::move(name)), backend_(backend) {}

	// Register a method with automatic type conversion
	template<typename ReturnType, typename... Args>
	ClassRegistration& Method(const std::string& method_name, std::function<ReturnType(Args...)> func) {
		...
		auto wrapper = [func, method_name](const std::vector<ScriptValue>& args) -> ScriptValue {
			if (args.size() != sizeof...(Args)) {
				...
				spdlog::warn(
					"[Scripting] Argument count mismatch for method '{}': expected {}, got {}",
					method_name,
					sizeof...(Args),
					args.size()
				);
	...
	template<typename ReturnType, typename... Args>
	ClassRegistration& StaticMethod(const std::string& method_name, std::function<ReturnType(Args...)> func) {
...
public:
	...
	explicit ScriptingSystem(ScriptLanguage language = ScriptLanguage::Lua) : language_(language) {
	...
	template<typename ReturnType, typename... Args>
	void RegisterGlobalFunction(const std::string& name, std::function<ReturnType(Args...)> func) {
		auto wrapper = [func, name](const std::vector<ScriptValue>& args) -> ScriptValue {
			if (args.size() != sizeof...(Args)) {
				// Argument count mismatch - log warning and return default value
				spdlog::warn(
					"[Scripting] Argument count mismatch for function '{}': expected {}, got {}",
					name,
					sizeof...(Args),
					args.size()
				);
	...
	template<typename ReturnType, typename... Args>
	void RegisterGlobalFunction(const std::string& name, ReturnType (*func)(Args...)) {
		RegisterGlobalFunction(name, std::function<ReturnType(Args...)>(func));
	}

	// Execute a script string
	bool ExecuteString(const std::string& script) { return backend_->ExecuteString(script); }

	// Execute a script file
	bool ExecuteFile(const std::string& filepath) { return backend_->ExecuteFile(filepath); }
	...
	[[nodiscard]] ScriptLanguage GetLanguage() const { return language_; }
