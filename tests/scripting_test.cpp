...
TEST_F(ScriptingSystemTest, can_handle_multiple_types) {
	...
	scripting.RegisterGlobalFunction("multiply", std::function<double(double, double)>([](double a, double b) {
										 return a * b;
									 }));
...
TEST_F(ScriptingSystemTest, can_handle_strings) {
	...
	scripting.RegisterGlobalFunction(
		"concat",
		std::function<std::string(std::string, std::string)>([](std::string a, std::string b) { return a + b; })
	);
...
TEST_F(ScriptingSystemTest, can_register_class_methods) {
	...
	scripting.RegisterClass("Math").Method(
									   "AddTwo",
									   std::function<int(int, int)>([](int a, int b) { return a + b; })
	).Method("Square", std::function<int(int)>([](int x) { return x * x; }));
...
TEST_F(ScriptingSystemTest, can_change_language_at_runtime) {
	...
	scripting.RegisterGlobalFunction("test_func", std::function<int(int)>([](int x) { return x * 2; }));
