...
protected:
	void SetUp() override { checkbox = std::make_unique<Checkbox>(10, 10, "Enable Sound"); }

	void TearDown() override { checkbox.reset(); }

	std::unique_ptr<Checkbox> checkbox;
};

TEST_F(CheckboxTest, Constructor_SetsLabel) { EXPECT_EQ(checkbox->GetLabel(), "Enable Sound"); }

TEST_F(CheckboxTest, Constructor_DefaultsToUnchecked) { EXPECT_FALSE(checkbox->IsChecked()); }
...
TEST_F(CheckboxTest, SetChecked_DoesNotTriggerCallback) {
	...
	checkbox->SetToggleCallback([&](bool checked) { callback_triggered = true; });
...
TEST_F(CheckboxTest, OnClick_LeftPressed_TogglesCheckbox) {
	...
	MouseEvent event{15, 15, false, false, true, true, 0.0f};
...
TEST_F(CheckboxTest, OnClick_OutsideBounds_DoesNotToggle) {
	...
	MouseEvent event{500, 500, false, false, true, true, 0.0f};
	checkbox->OnClick(event);
...
TEST_F(CheckboxTest, IsVisible_DefaultsToTrue) { EXPECT_TRUE(checkbox->IsVisible()); }
