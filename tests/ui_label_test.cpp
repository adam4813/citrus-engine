...
protected:
	void SetUp() override { label = std::make_unique<Label>(10, 10, "Test Label"); }

	void TearDown() override { label.reset(); }

	std::unique_ptr<Label> label;
};

TEST_F(LabelTest, Constructor_SetsText) { EXPECT_EQ(label->GetText(), "Test Label"); }
...
TEST_F(LabelTest, IsVisible_DefaultsToTrue) { EXPECT_TRUE(label->IsVisible()); }
