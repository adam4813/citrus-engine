...
struct TileDescription {
	...
	TileDescription(const uint32_t tile_id, const glm::vec4& rect) : id(tile_id), texture_rect(rect) {}
};
...
public:
	...
	[[nodiscard]] const TileDescription* GetTile(uint32_t id) const;

	// Set the image path for this tileset (instead of TextureId)
	void SetImagePath(const std::string& image_path);

	// Get the image path
	[[nodiscard]] const std::string& GetImagePath() const { return image_path_; }

	// Get all tile descriptions
	[[nodiscard]] const std::vector<TileDescription>& GetTiles() const { return tiles_; }

	// Get tile size in pixels (assuming uniform tile size)
	[[nodiscard]] glm::ivec2 GetTileSize() const { return tile_size_; }
