...
void Tileset::AddTile(uint32_t id, const glm::vec4& texture_rect) {
	...
	const auto it = std::ranges::find_if(tiles_, [id](const TileDescription& tile) { return tile.id == id; });
...
void Tileset::SetImagePath(const std::string& image_path) { image_path_ = image_path; }
} // namespace engine::assets
