...
protected:
	void SetUp() override { slider = std::make_unique<Slider>(10, 10, 200, 30, 0.0f, 100.0f, 50.0f); }

	void TearDown() override { slider.reset(); }

	std::unique_ptr<Slider> slider;
};

TEST_F(SliderTest, Constructor_SetsInitialValue) { EXPECT_EQ(slider->GetValue(), 50.0f); }
...
TEST_F(SliderTest, SetValue_DoesNotTriggerCallback) {
	...
	slider->SetValueChangedCallback([&](float value) { callback_triggered = true; });
...
TEST_F(SliderTest, OnDrag_UpdatesValue) {
	...
	// Start dragging
	MouseEvent click_event{10, 25, false, false, true, true, 0.0f};
	slider->OnClick(click_event);

	callback_triggered = false; // Reset flag

	// Drag to right side (should move value towards max)
	MouseEvent drag_event{200, 25, true, false, false, false, 0.0f};
