...
void SceneRegistry::RegisterScene(const std::string& name,
                                  const std::string& description,
                                  SceneFactory factory) {
    // Check if scene already registered
    auto it = std::find_if(scenes_.begin(), scenes_.end(),
        [&name](const SceneInfo& info) { return info.name == name; });
...
const std::vector<SceneInfo>& SceneRegistry::GetAllScenes() const {
    return scenes_;
}
