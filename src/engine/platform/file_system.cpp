...
bool File::Open(const Path& path, const FileMode mode, FileType type) {
	const std::string mode_str = (mode == FileMode::Read)     ? "r"
								 : (mode == FileMode::Write)  ? "w"
								 : (mode == FileMode::Append) ? "a"
															  : "r+";
...
void File::Close() { file_stream_.close(); }

bool File::IsOpen() const { return file_stream_.is_open(); }
...
std::vector<uint8_t> File::ReadAll() {
	return file_stream_.eof() ? std::vector<uint8_t>()
							  : std::vector<uint8_t>(std::istreambuf_iterator(file_stream_), {});
}

std::string File::ReadText() {
	return file_stream_.eof() ? std::string() : std::string(std::istreambuf_iterator(file_stream_), {});
}
...
Path GetAssetsDirectory() { return Path("assets"); }

bool Exists(const Path& path) { return std::filesystem::exists(path); }
