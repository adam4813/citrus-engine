...
bool Shader::IsValid() const { return pimpl_->valid; }
...
ShaderId ShaderManager::CreateShader(const std::string& name) const {
	// Check if shader with this name already exists
	if (const auto existing = FindShader(name); existing != INVALID_SHADER) {
		return existing;
	}

	// Allocate a new shader slot (uncompiled)
...
bool ShaderManager::CompileShader(
	...
	return CompileShader(id, *vertex_src_opt, *fragment_src_opt);
}
