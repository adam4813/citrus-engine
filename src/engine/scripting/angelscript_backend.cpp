...
private:
	...
	void PushValue(asIScriptContext* ctx, int arg_index, const ScriptValue& value) {
		if (value.Is<int>()) {
			...
			throw std::invalid_argument(
				"AngelScriptBackend: String arguments are not supported. Please avoid passing "
				"std::string to scripts, or implement proper string registration."
			);
		}
	}
	...
	static void GenericFunctionWrapper(asIScriptGeneric* gen) {
		...
		auto* func_ptr = static_cast<std::function<ScriptValue(const std::vector<ScriptValue>&)>*>(gen->GetAuxiliary());
...
public:
	...
	[[nodiscard]] ScriptLanguage GetLanguage() const override { return ScriptLanguage::AngelScript; }

	~AngelScriptBackend() override { Shutdown(); }
};

// Factory helper function
std::unique_ptr<IScriptingBackend> CreateAngelScriptBackend() { return std::make_unique<AngelScriptBackend>(); }
} // namespace engine::scripting
