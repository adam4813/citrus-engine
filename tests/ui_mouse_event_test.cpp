...
public:
	TestUIElement(float x, float y, float w, float h) : UIElement(x, y, w, h) {}
...
public:
	CountingUIElement(float x, float y, float w, float h) : UIElement(x, y, w, h) {}
...
TEST_F(UIMouseEventTest, MouseEventManagerHitTesting) {
	...
	manager.RegisterRegion(Rectangle{0.0f, 0.0f, 100.0f, 100.0f}, [&handler1_called](const MouseEvent&) {
		handler1_called = true;
		return false;
	});

	// Region 2: Bottom-right quadrant
	manager.RegisterRegion(Rectangle{200.0f, 200.0f, 100.0f, 100.0f}, [&handler2_called](const MouseEvent&) {
...
TEST_F(UIMouseEventTest, MouseEventManagerUpdateBounds) {
	...
	auto handle = manager.RegisterRegion(Rectangle{0.0f, 0.0f, 100.0f, 100.0f}, [&handler_called](const MouseEvent&) {
		handler_called = true;
		return false;
	});
