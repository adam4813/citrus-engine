...
private:
	...
	ScriptValue PopValue() {
		...
		case LUA_TNUMBER:
			...
			break;
		case LUA_TBOOLEAN: result.value = static_cast<bool>(lua_toboolean(lua_state_, -1)); break;
		case LUA_TSTRING: result.value = std::string(lua_tostring(lua_state_, -1)); break;
		case LUA_TNIL:
		default:
	...
	static int FunctionDestructor(lua_State* L) {
		auto* ptr = static_cast<std::function<ScriptValue(const std::vector<ScriptValue>&)>*>(lua_touserdata(L, 1));
		ptr->~function();
		return 0;
	}
	...
	static int LuaCFunctionWrapper(lua_State* L) {
		...
		for (int i = 1; i <= argc; ++i) {
			...
			case LUA_TNUMBER:
				...
				break;
			case LUA_TBOOLEAN: arg.value = static_cast<bool>(lua_toboolean(L, i)); break;
			case LUA_TSTRING: arg.value = std::string(lua_tostring(L, i)); break;
			default:
				// For nil/unsupported types, leave as empty any
				break;
			}
...
public:
	...
	bool ExecuteFile(const std::string& filepath) override {
		...
		if (luaL_dofile(lua_state_, filepath.c_str()) != LUA_OK) {
			...
			const char* error_msg = lua_tostring(lua_state_, -1);
			spdlog::error(
				"[Lua] Script file execution failed ({}): {}",
				filepath,
	...
	void RegisterGlobalFunction(
		...
		if (reinterpret_cast<uintptr_t>(raw_ptr) % func_align != 0) {
			spdlog::error(
				"[Lua] Alignment check failed for function userdata. Expected alignment: {}, got address: {}",
				func_align,
	...
	[[nodiscard]] ScriptLanguage GetLanguage() const override { return ScriptLanguage::Lua; }

	~LuaBackend() override { Shutdown(); }
};

// Factory helper function
std::unique_ptr<IScriptingBackend> CreateLuaBackend() { return std::make_unique<LuaBackend>(); }
} // namespace engine::scripting
