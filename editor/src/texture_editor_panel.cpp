...
void TextureEditorPanel::UpdatePreview() { preview_color_ = EvaluateOutputColor(); }
		...
		if (node.type_name == "Texture Output") {
			...
			for (const auto& link : texture_graph_->GetLinks()) {
				if (link.to_node_id == node.id && link.to_pin_index == 0) {
					...
					if (const auto* source_node = texture_graph_->GetNode(link.from_node_id);
						source_node && source_node->type_name == "Solid Color") {
