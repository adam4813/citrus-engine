...
static const engine::graph::Link* FindInputLink(const engine::graph::NodeGraph& graph, int node_id, int pin_index) {
...
glm::vec4 TextureEditorPanel::SampleBuffer(int node_id, int output_pin, glm::vec2 uv) const {
	...
	if (const auto* node = texture_graph_->GetNode(node_id); node && node->type_name == "Channel Split") {
		const float ch = output_pin == 0   ? sample.r
						 : output_pin == 1 ? sample.g
						 : output_pin == 2 ? sample.b
										   : sample.a;
	...
	if (type == "Input Image") {
		...
		for (int y = 0; y < res; ++y) {
			for (int x = 0; x < res; ++x) {
				...
				glm::vec2 sample_uv = IsConnected(*texture_graph_, node_id, 0) ? SampleInputVec2(*node, 0, uv) : uv;
				...
				buf.pixels[y * res + x] = glm::vec4(
					img.pixels[idx + 0] / 255.0f,
					img.pixels[idx + 1] / 255.0f,
					img.pixels[idx + 2] / 255.0f,
					img.pixels[idx + 3] / 255.0f
				);
			}
		}
		return;
...
void TextureEditorPanel::UploadNodeThumbnails() {
	...
	for (auto& [id, buf] : node_buffers_) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, thumb_res, thumb_res, 0, GL_RGBA, GL_UNSIGNED_BYTE, thumb_data.data());
	}
